...
bg.Image = "rbxassetid://11902680347" -- Makima Decal
...
local function mkCard(labelText)
    ...
    local lab = mkLabel(card, labelText, 12, true); lab.TextTransparency = 0.15; lab.Position = UDim2.new(0, 10, 0, 6); lab.ZIndex = 1
    local val = mkLabel(card, "…", 18, true); val.Position = UDim2.new(0, 10, 0, 26); val.ZIndex = 1
    return card, val, s
end
...
local function updateRainbow(strokeObj)
    local t = tick() % 5 / 5
    strokeObj.Color = Color3.fromHSV(t, 1, 1)
end
